...
void ActiveLook::sendCommand(Command cmd, std::span<const uint8_t> payload) {
    ...
    for (const uint16_t h : COMMAND_HANDLES) {
        ble_gattc_write_no_rsp_flat(*connectionHandle_, h, buf.data(),
                                    buf.size());
    }
}
...
void ActiveLook::displayText(std::string_view msg) {
    ...
    for (const char c : displayMsg) txt.push_back(static_cast<uint8_t>(c));
    txt.push_back('\0');
